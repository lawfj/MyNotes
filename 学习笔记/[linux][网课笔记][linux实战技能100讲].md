...
添加SGID
chmod 1544 test1
[root@10-255-0-194 tmp]# chmod 1544 test1
[root@10-255-0-194 tmp]# ls -ld test1
-r-xr--r-T 1 root root 0 Aug  9 17:44 test1
...
## 37 | 使用ps和top命令查看进程
进程：运行中的程序
进程管理：什么时候启动、生命周期、资源、如何结束、进程间通信等等
• 进程的概念与进程查看
• 进程的控制命令
• 进程的通信⽅式— 信号
• 守护进程和系统⽇志
...
ps中的time不具有参考价值
UID：对应的是有效用户ID，并不是启动进程的用户哦，因为可以以某个用户的身份去启动进程
PID：进程id
PPID：该进程的父进程
### 常用
```
